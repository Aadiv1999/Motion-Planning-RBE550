...
class Visualizer:
    ...
    def display_robot(self):
        ...
        self.path.append([self.robot.x_coord, 900-self.robot.y_coord])

        for i in self.path:
            pygame.draw.circle(self.screen, self.BLUE, i, 2)

        coor = 'Coordinates: ' + str(np.round(self.robot.get_position(),2))
        speeds = 'Speeds: ' + str(np.round(self.robot.get_speed(),2))
        angle = 'Heading: ' + str(np.round(self.robot.angle,2))
        ...
        text3 = self.font.render(angle, True, self.BLACK)
        self.screen.blit(text3, (1, 55))
...
class Runner:
    ...
    def run(self):
        ...
        while running:
            ...
            # update x,y,theta of robot
            self.controller.step()

            success = self.controller.check_success(point)
            ...
            if success:
                ...
                if abs(curr_angle - desired_angle) > 0.05:
                    self.robot.turn(curr_angle - desired_angle, 0.05)
                else:
                    self.robot.move(0.05, 0.05)
