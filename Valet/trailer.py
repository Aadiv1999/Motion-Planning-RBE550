...
class Robot:
    ...
    def get_robot_points(self):
        ...
        # right side
        points.append([0, self.height/2]) # index 5
        points.append([self.width, self.height/2]) # index 6
        # left wheel
        points.append([0, -self.height/2]) # index 7
        points.append([self.width, -self.height/2]) # index 8
        ...
        points.append([0, self.rad]) # index 15
        points.append([0, -self.rad]) # index 16

        # trailer points
        # index 17 - 21
        points.append(rot_points([0,0], self.theta))
        points.append(rot_points([-self.width/2, self.height/2], self.theta-self.angle))
        points.append(rot_points([-self.width, self.height/2], self.theta-self.angle))
        points.append(rot_points([-self.width, -self.height/2], self.theta-self.angle))
        points.append(rot_points([-self.width/2, -self.height/2], self.theta-self.angle))
    ...
    def move(self, speed: float, phi: float = 0) -> None:
        ...
        if self.angle - self.theta > 180:
            t = abs((360 + self.angle - self.theta)) * pi/180 + 180
        else:
            t = abs((-self.angle + self.theta) * pi/180)
...
class Controller:
    ...
    def step(self):
        ...
        self.robot.theta += atan2((self.robot.vel_rt - self.robot.vel_lt),self.robot.height)*180/pi * self.robot.dt
        self.robot.theta = self.robot.theta % 360

        # print(self.robot.angle-self.robot.theta)
...
class Visualizer:
    ...
    def display_robot(self):
        ...
        for i in range(17,len(robot_points)):
            if i == 21:
                pygame.draw.line(self.screen, self.RED, robot_points[i], robot_points[17], 4)
            else:
                pygame.draw.line(self.screen, self.RED, robot_points[i], robot_points[i+1], 4)
        ...
        self_theta = 'Trailer: ' + str(np.round(self.robot.angle-self.robot.theta,2))
        ...
        text4 = self.font.render(self_theta, True, self.BLACK)
        self.screen.blit(text4, (1, 105))
